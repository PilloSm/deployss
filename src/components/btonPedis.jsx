...
export default function BtonPedir({ carrito, id_cuenta, saldo }) {
  ...
  const handleSubmit = async () => {
    if (datos.total <= saldo && saldo !== 0) {
      const res = await axios.post("/api/apiCliente/pedido", datos);
  ...
  return (
    <button
      ...
      className="absolute w-[528px] h-[95px] top-[759px] left-[426px] bg-[#25a18ee6] hover:bg-[#5cc0b1e6] rounded-full border-none cursor-pointer"
    >
